...
def digitilize_multi_line(
    ...
    * k, b - coefficient from regression which translates pixel value inside
    ...
    # Cluster anomaly pixels as there may be lines of different colors in one plot
    return cropped_img[cluster_labels == 1], np.argwhere(cluster_labels == 1)
...
def digitilize_single_line(
    ...
    * k, b - coefficient from regression which translates pixel value inside
    ...
    for i in range(cluster_labels.shape[0]):
        ...
        if len(ixs) > 0:
            x_values.append(
                mnemonic_per_pixel[int(np.median(ixs))]
            )
            y_values.append(
                depth_per_pixel[i]
            )
